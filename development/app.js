  ...
  dashboard: () =>
    m(
      ...
      m(
        ...
        state.lastSync &&
          m("span", "Tersinkronisasi " + moment(state.lastSync).fromNow())
      ),
      _.chunk(
        _.map( state.login.bidang === 1 ? _.pick(menus,['registration']) : 
              state.login.bidang === 2 ? _.pick(menus,['cashier']) :               
              state.login.bidang === 3 ? _.pick(menus,['storage']) :               
              state.login.bidang === 4 ? _.pick(menus,['pharmacy']) :               
              state.login.bidang === 5 ? menus :               
              state.login.bidang === 6 ? _.pick(menus,['registration','outpatient']) :               
              _.pick(menus,[])
              , (v, k) => [v, k]),
        3
      ).map((i) =>
      ...
      m(
              ...
              _.merge(state, {
                stats: {
                  pasien: {
                    ...
                    wanita: array.filter((i) => i.identitas.kelamin === 2)
                      .length,
                  },
                  ...
                  emergency: _.sum(
                    array.map((i) => (i.emergency || []).length)
                  ),
                  rawatInap: _.sum(
                    array.map((i) => (i.rawatInap || []).length)
                  ),
                  radiology: _.flattenDeep(
                    array.map((i) =>
                      [
                        ...(i.rawatJalan || []),
                        ...(i.emergency || []),
                        ...
                        .map((j) => _.get(j, "soapDokter.radio"))
                        .filter(Boolean)
                    )
                  ).length,
                  laboratory: _.flattenDeep(
                    array.map((i) =>
                      [
                        ...(i.rawatJalan || []),
                        ...(i.emergency || []),
                        ...
                        .map((j) => _.get(j, "soapDokter.labor"))
                        .filter(Boolean)
                    )
                  ).length,
                },
        ...
        },
        {
          ...
          emergency: [
            "Total pasien emergency: " + _.get(state, "stats.emergency"),
          ],
          rawatInap: [
            "Total pasien pernah inap: " + _.get(state, "stats.rawatInap"),
          ],
          radiology: [
            "Total riwayat layanan radiologi: " +
              _.get(state, "stats.radiology"),
          ],
          laboratory: [
            "Total riwayat layanan laboratorium: " +
              _.get(state, "stats.laboratory"),
          ],
          storage: [
        ...
        }[state.dashboardTab || "pasien"].map((i) =>
          m(".column", m(".notification", { class: "is-primary" }, i))
        )
      )
    ),
  ...
  login: () =>
    m(
      ...
      m(
        ...
        m(
          ".column",
          !ors([window.chrome, typeof InstallTrigger === "object"]) &&
            m(
              ".notification.is-warning.is-light",
              "Mohon gunakan Chrome/Firefox terbaru"
            ),
          state.error &&
            m(".notification.is-danger.is-light", [
          ...
          m(
            ...
            m(
              ...
              m(
                ...
                m("i.fas.fa-8x.fa-hospital-user")
              )
            )
          ),
          m("br"),
          ...
          m(
            autoForm({
              ...
              action: (doc) => [
                (state.loading = true),
                m.redraw(),
                io().emit("login", _.assign(doc, {username: _.lowerCase(doc.username)}), ({ res }) =>
                  res
                    ? [
                        _.assign(state, {
...
io().on("connect", (socket) => [
  (state.login = localStorage.login && JSON.parse(localStorage.login || "{}")),
