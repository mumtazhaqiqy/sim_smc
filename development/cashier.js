...
_.assign(comp, {
  cashier: () =>
    state.login.bidang !== 2
      ? m("p", "Hanya untuk user bidang kasir")
      : m(
          ".content",
          ...
          m(
            ...
            m(
              ...
              makeIconLabel(
                "filter",
                state.todayPoliFilter ? "Seluruh antrian" : "Saring hari ini"
              )
            )
          ),
                      ...
                      withThis(
                        ...
                        (cashierFilter) =>
                          db.patients
                            ...
                            .toArray((array) => [
                              (state.cashierList = array.flatMap((i) =>
                                [
                                  ...(i.rawatJalan || []),
                                  ...(i.emergency || []),
                                  ...(i.rawatInap || []),
                            ...
                            withThis(
                              [
                                !rawat.bed
                                  ...
                                  : [],
                                ors([
                                  // tarif layanan rawat (jalan/inap/igd)
                                  !rawat.bayar_pendaftaran,
                                  ...
                                  ? ors([
                                      rawat.klinik && [
                                        ...
                                        1000 *
                                          +look("tarif_klinik", rawat.klinik),
                                      ],
                                      rawat.bed && [
                                        "Biaya Kamar " +
                                          _.values(rawat.bed)
                                  ...
                                  : [],
                                ...(ors([
                                  // tampilkan jika salah 1 kondisi ini terpenuhi
                                  ands([
                                    rawat.klinik,
                                    rawat.soapDokter,
                                    !rawat.bayar_konsultasi,
                                  ]), // keluar klinik dan belum bayar
                                  ands([
                                    rawat.bed,
                                    rawat.keluar,
                                    !rawat.bayar_konsultasi,
                                  ]), // sudah pulang inap dan belum bayar
                                  ands([
                                    rawat.soapDokter,
                                    !rawat.bayar_konsultasi,
                                  ]), // keluar IGD dan belum bayar
                                ])
                                  ? [
                                      ...[
                                        // daftar tindakan
                                ...
                                .filter((k) => k.length)
                                .map((k) => ({ item: k[0], harga: k[1] })),
                              (bills) =>
                                (state.modalCashier = m(
                                  ".box",
                                  ...
                                  m(
                                    ...
                                    m(
                                      ".button.is-success",
                                      {
                                        onclick: () => [
                                          confirm(
                                            "Yakin sudah menerima pembayaran?"
                                          ) &&
                                            updateBoth(
                                              "patients",
                                              pasien._id,
  ...
  overcharge: () =>
    m(
      ...
      m(
        autoForm({
          id: "overcharge",
          schema: schemas.overcharge,
          layout: { "charges.$": [["item", "harga"]] },
          action: (doc) =>
            updateBoth(
              // selipkan catatan tambahan biaya pada rawat
              "patients",
              state.onePatient._id,
              _.assign(state.onePatient, {
                rawatJalan: (state.onePatient.rawatJalan || []).map((i) =>
                  i.idrawat === state.oneRawat.idrawat ? _.assign(i, doc) : i
                ),
                emergency: (state.onePatient.emergency || []).map((i) =>
                  i.idrawat === state.oneRawat.idrawat ? _.assign(i, doc) : i
                ),
                rawatInap: (state.onePatient.rawatInap || []).map((i) =>
                  i.idinap === state.oneRawat.idinap ? _.assign(i, doc) : i
                ),
              }),
              (res) =>
                res && [
